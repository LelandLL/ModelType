...
public class Lv1CannonScript : MonoBehaviour {
    ...
    void Update()
    {
        if (Input.GetButtonDown("Fire1") && !Pause.gamePaused)
        {
            //transform.parent.GetComponent<AudioSource>().Play();
            int wepLvl = transform.parent.gameObject.GetComponent<Controller>().weaponLevel;
            if (wepLvl == 1)
            {
